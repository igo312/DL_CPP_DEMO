...
std::shared_ptr<NetworkRunner> FrontDetectorBuilder::getRunner(){
    ...
    auto outputDims_ = engine_->GetBindingDimensions(1);
    ...
    // 输出的总大小计算
    const int size = outputDims_.d[0] * outputDims_.d[1] * outputDims_.d[2];
    
    // 输出内存的分配 device
    CHECK(cudaMalloc(&(runner->d_output_), max_batch_ * size * sizeof(float)));
    CHECK(cudaMalloc(&(runner->d_output_post_), max_batch_ * outputDims.d[0] * outputDims_0.d[2] * runner->m_max_det * sizeof(float)));
...
void FrontDetectorRunner::infer(void* image, int batch_size, int image_width, int image_height){
    ...
    // 推理
    timer.start();
    context_->Enqueue(batch_size, d_input_, stream_, nullptr);
    cudaStreamSynchronize(stream_);
    timer.stop();
    time_infer += timer.last_elapsed()
    // 后处理
...
void FrontDetectorRunner::prerpocess(int batch_size, int image_width, int image_height){
    ...
    for( int i = 0; i < batch_size; i++){
        ...
        int img_w = scale * frame->width;
        int img_h = scale * frame->height;
...
void FrontDetectorRunner::postprocess(int batch_size){
    ...
    non_max_suppression(d_output_, batch_size, num_bboxes, num_classes,
                         m_conf_thres, m_iou_thres, d_output_post_,
                         m_max_det, stream_);
}

void FrontDetectorRunner::~FrontDetectorRunner(){
    if ( d_input_ != nullptr ){
        cudaFree(d_input_);
    }
